    ...
    public class SubmissionController : Controller
    {
        ...
        public async Task<IActionResult> Create(string AVNumber)
        {
            var submissionModel = new SubmissionCreateViewModel
            {
                ...
                Senders = new List<SenderViewModel>(),
                Organisations = new List<SenderViewModel>()
            };
        ...
        public async Task<IActionResult> Create(SubmissionCreateViewModel submission)
        {
            if (!ModelState.IsValid)
            {
                ...
                submission.Senders = new List<SenderViewModel>();
                submission.Organisations = new List<SenderViewModel>();
        ...
        public async Task<IActionResult> Edit(string AVNumber)
        {
            ...
            submissionModel.Senders = new List<SenderViewModel>();
            submissionModel.Organisations = new List<SenderViewModel>();
        ...
        public async Task<PartialViewResult> GetSenderDetails(Guid? countryId)
        {
            ...
            var senderModel = _mapper.Map<List<SenderViewModel>>(senderDto);
        ...
        [HttpPost]
        public async Task<JsonResult> AddSender(SenderViewModel senderModel)
        {
            if (ModelState.IsValid)
            {
        ...
        {
            var viabilityLookup = await _lookupService.GetAllCountriesAsync();
        ...
        {
            var viabilityLookup = await _lookupService.GetAllSubmittingLabAsync();
            return viabilityLookup.Select(f => new SelectListItem { Value = f.Id.ToString(), Text = f.Name }).ToList();
        }

        private async Task<List<SelectListItem>> GetSubmissionReasonDropdownList()
        {
            var viabilityLookup = await _lookupService.GetAllSubmissionReasonAsync();
            return viabilityLookup.Select(f => new SelectListItem { Value = f.Id.ToString(), Text = f.Name }).ToList();
        }
    }
}
